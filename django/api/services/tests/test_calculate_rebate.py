...
from ..calculate_rebate import calculate_individual_rebate_amount, get_cra_results_individuals_only
...
class TestCalculate(TestRebate):
    ...
    def test_individual_receives_rebate_a(self):
        ...
        filtered_applications = GoElectricRebateApplication.objects.filter(id__in=list(cra_response.keys()))
        for each in filtered_applications:
            single_cra_response = cra_response.get(each.id)
            rebate_amount = calculate_individual_rebate_amount(single_cra_response, each)
            self.assertEqual(
    ...
    def test_individual_receives_not_approved_high_income(self):
        ...
        application = GoElectricRebateApplication.objects.filter(id__in=list(cra_response.keys())).first()
        single_cra_response = cra_response.get(application.id)
        rebate_amount = calculate_individual_rebate_amount(single_cra_response, application)
        self.assertEqual(
    ...
    def test_individual_receives_not_approved_no_cra_info(self):
        ...
        rebate_amount = calculate_individual_rebate_amount(single_cra_response, application)
        self.assertEqual(
            rebate_amount, RebateType.E.value
        )
    ...
    def test_individual_receives_not_approved_sin_mismatch(self):
        ...
        rebate_amount = calculate_individual_rebate_amount(single_cra_response, application)
        self.assertEqual(
            rebate_amount, RebateType.F.value
        )
